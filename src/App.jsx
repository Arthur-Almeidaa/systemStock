...
const resolveSetor = (s) => IS_FERR_SUB(s) ? FERRAMENTAS_SUB[s] : SETORES[s];
...
export default function App() {
  ...
  if (isPedidosUser(user)) return <PedidosScreen user={user} onLogout={logout}/>;
  ...
  if (!setor) return (
    ...
    <div className="app">
      <header className="header">
        ...
        <div className="header-right"><span className="header-email">{user.email}</span><button className="hbtn danger" onClick={logout}><Icon name="logout" size={14}/> SAIR</button></div>
      </header>
      {showFerrSub?<FerramentasSubScreen onSelect={selectFerrSub} onBack={()=>setShowFerrSub(false)}/>:<SetorScreen user={user} onSelect={selectSetor}/>}
    </div>
    <Toast toasts={toasts}/></>
  );
